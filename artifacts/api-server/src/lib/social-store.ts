...
export interface SocialDataset {
  ...
  syncState?: Record<string, { lastSynced?: string; warnings?: string[] }>;
}
...
function demo(): SocialDataset {
  ...
  return {generatedAt:new Date().toISOString(),isDemo:true,monthlyMetrics,contents,campaigns,interactions,platforms:platforms.map(p=>({platform:p,followers:0,growth:0,views:contents.filter(x=>x.platform===p).reduce((s,x)=>s+x.views,0),reach:contents.filter(x=>x.platform===p).reduce((s,x)=>s+x.reach,0),engagement:contents.filter(x=>x.platform===p).reduce((s,x)=>s+x.engagement,0),posts:contents.filter(x=>x.platform===p&&x.type!=='Reel').length,reels:contents.filter(x=>x.platform===p&&x.type==='Reel').length,stories:0,messages:contents.filter(x=>x.platform===p).reduce((s,x)=>s+x.messages,0)})),syncState:{}};
}
...
export function rebuildAggregates(data:SocialDataset):SocialDataset{
  ...
  const platforms=names.map(p=>{const rows=contents.filter(x=>x.platform===p);const previous=data.platforms.find(x=>x.platform===p);return{platform:p,followers:previous?.followers??0,growth:previous?.growth??0,views:rows.length?rows.reduce((s,x)=>s+x.views,0):previous?.views??0,reach:rows.length?rows.reduce((s,x)=>s+x.reach,0):previous?.reach??0,engagement:rows.length?rows.reduce((s,x)=>s+x.engagement,0):previous?.engagement??0,posts:rows.filter(x=>x.type==='Post'||x.type==='Threads Post').length,reels:rows.filter(x=>x.type==='Reel').length,stories:rows.filter(x=>x.type==='Story').length,messages:rows.length?rows.reduce((s,x)=>s+x.messages,0):previous?.messages??0}});
