...
import { calculateCampaignMetrics, campaignTopics, contentsForCampaign, crossPublishedGroups, dataQuality, growthForContent, metricDisplay, peopleMetricValue, recommendationFromTopics } from '@/lib/analytics';
import { classifyInquiry, classifyInteractionCampaign, INQUIRY_TOPICS } from '@/lib/classifier';
import { detectImportKind, normalizeImportedContent, normalizeImportedInteraction, normalizeImportedMonthlyMetric, normalizeImportedPlatformMetric, normalizeMetaContentExport, parseMessengerConversationText, parseThreadsCapturePayload, parseThreadsInsightsText, type ImportKind, type ThreadsCapturePayload } from '@/lib/importer';
import { discoverCampaignsFromContents } from '@/lib/campaign-discovery';
import { fetchBackendDataset, fetchBackendStatus, fetchOAuthPublicConfig, getBackendUrl, requestBackendSync, setBackendUrl, type OAuthPublicConfig } from '@/lib/backend';
...
function contentMetricText(content: SocialContent, metric: 'views' | 'reach' | 'impressions') {
  const status = metricDisplay(content, metric);
  return status.available ? formatCompact(status.value ?? 0) : '未提供';
}
...
function monthlyFromContents(contents: SocialContent[], year: string) {
  ...
  for (const content of byYear(contents, year)) {
    ...
    rows[month].reach += peopleMetricValue(content);
...
function Overview() {
  ...
  const totals = useMemo(() => ({
    ...
    reach: yearContents.reduce((s, x) => s + peopleMetricValue(x), 0) || monthly.reduce((s, x) => s + x.reach, 0),
  ...
  const campaigns = useMemo(() => data.campaigns.map((campaign) => ({ campaign, metrics: calculateCampaignMetrics(data, campaign) })).sort((a, b) => b.metrics.peopleCount - a.metrics.peopleCount), [data]);
  ...
  const platformRows = ['Facebook', 'Instagram', 'Threads'].map((platform) => ({ platform, reach: yearContents.filter((x) => x.platform === platform).reduce((s, x) => s + peopleMetricValue(x), 0), engagement: yearContents.filter((x) => x.platform === platform).reduce((s, x) => s + x.engagement, 0) }));
  ...
  return <Shell title="年度總覽">
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">{loading ? [1,2,3,4].map((x) => <div key={x} className="h-[142px] animate-pulse rounded-2xl bg-secondary" />) : <><Kpi label="已取得觀看 / 曝光" value={formatCompact(totals.views)} note={`${yearContents.filter(x=>metricDisplay(x,'views').available).length} 則有觀看欄位；缺欄位不視為 0`} /><Kpi label="人數型指標" value={formatCompact(totals.reach)} note="FB/IG 觸及＋Threads 瀏覽人數；逐篇加總、非去重" /><Kpi label="有效互動" value={formatCompact(totals.engagement)} note="按讚、留言、分享等" /><Kpi label="私訊 / 詢問" value={formatNumber(totals.messages)} note="與 conversation count 分開保存" accent="text-accent" /></>}</div>
    <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-[1.65fr_1fr]">
      <Card className="min-h-[390px]"><ChartHeader title="社群人數型指標與互動趨勢" eyebrow="全年節奏" rows={monthly.map((r) => ({ 月份:r.month, 觸及:r.reach, 互動:r.engagement, 私訊:r.messages }))} filename={`${year}-monthly.csv`} /><div className="h-[315px] px-3 pb-4 pt-4 sm:px-5"><ResponsiveContainer width="100%" height="100%"><LineChart data={monthly} margin={{ top:10,right:8,left:-18,bottom:0 }}><CartesianGrid stroke="hsl(var(--border))" strokeDasharray="3 5" vertical={false}/><XAxis dataKey="month" tick={{fontSize:11}} tickLine={false} axisLine={false}/><YAxis tick={{fontSize:11}} tickFormatter={formatCompact} tickLine={false} axisLine={false}/><Tooltip content={<TooltipBox/>}/><Legend wrapperStyle={{fontSize:11,paddingTop:12}}/><Line type="monotone" dataKey="reach" name="人數型指標" stroke={COLORS.teal} strokeWidth={2.5} dot={false}/><Line type="monotone" dataKey="engagement" name="互動" stroke={COLORS.coral} strokeWidth={2.5} dot={false}/><Line type="monotone" dataKey="messages" name="私訊" stroke={COLORS.gold} strokeWidth={2} dot={false}/></LineChart></ResponsiveContainer></div></Card>
      <Card><ChartHeader title="平台貢獻" eyebrow="依實際內容加總" rows={platformRows.map((x) => ({平台:x.platform,觸及:x.reach,互動:x.engagement}))} filename={`${year}-platform.csv`} /><div className="flex h-[315px] flex-col justify-center gap-5 px-5 pb-5 pt-3"><div className="relative mx-auto h-[180px] w-full max-w-[250px]"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={platformRows} dataKey="reach" nameKey="platform" innerRadius={58} outerRadius={82} paddingAngle={3} stroke="none">{platformRows.map((entry) => <Cell key={entry.platform} fill={platformColors[entry.platform]} />)}</Pie><Tooltip content={<TooltipBox/>}/></PieChart></ResponsiveContainer><div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center"><span className="font-display text-[29px]">{formatCompact(totals.reach)}</span><span className="text-[10px] text-muted-foreground">人數型指標</span></div></div><div className="grid grid-cols-3 gap-2">{platformRows.map((p) => <div key={p.platform} className="text-center"><div className="mb-1 flex items-center justify-center gap-1 text-[11px] font-semibold"><span className="h-2 w-2 rounded-full" style={{backgroundColor:platformColors[p.platform]}}/>{p.platform}</div><p className="font-mono-ui text-[12px]">{formatCompact(p.reach)}</p></div>)}</div></div></Card>
    </div>
    <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-[1.1fr_1fr]">
      <Card><ChartHeader title="活動表現" eyebrow="由內容即時計算" rows={campaigns.map((x) => ({活動:x.campaign.name,人數型指標:x.metrics.peopleCount,互動:x.metrics.engagement}))} filename={`${year}-campaigns.csv`} /><div className="divide-y divide-border px-5 pb-2 pt-3">{campaigns.map((x,index) => <div key={x.campaign.id} className="flex items-center gap-4 py-3"><span className="font-mono-ui text-[11px] text-muted-foreground">{String(index+1).padStart(2,'0')}</span><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><p className="truncate text-[13px] font-semibold">{x.campaign.name}</p><span className="font-mono-ui text-[12px]">{formatCompact(x.metrics.peopleCount)}</span></div><div className="mt-2"><BarMeter value={x.metrics.peopleCount} max={campaigns[0]?.metrics.peopleCount || 1}/></div></div><Pill tone={index===0?'good':'neutral'}>{x.metrics.contentCount} 則</Pill></div>)}</div></Card>
...
function CampaignDetail({ campaign }: { campaign: Campaign }) {
  ...
  const crossGroups = crossPublishedGroups(metrics.contents);
  const maxContribution = Math.max(1, ...Object.values(metrics.platforms).map((x) => x.people));
  return <Shell title={campaign.name} section="活動分析"><PageIntro kicker="Campaign analysis" title={campaign.name} description={campaign.summary ?? '尚未補充活動摘要。'}><div className="flex gap-2 print-hidden"><PrintButton /></div></PageIntro><div className="mb-4 flex flex-wrap items-center gap-2 text-[11px] text-muted-foreground"><Pill tone="good">動態計算</Pill>{campaign.autoGenerated&&<Pill tone="warn">系統自動建立 · 請確認日期/關鍵字</Pill>}<span>{formatDate(campaign.startDate)} — {formatDate(campaign.endDate)}</span><span className="text-border">·</span><span>{metrics.contentCount} 則有效內容</span></div>
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><Kpi label="活動人數型指標" value={formatCompact(metrics.peopleCount)} note="FB/IG 觸及＋Threads 瀏覽人數；逐篇加總、非去重"/><Kpi label="已取得觀看" value={formatCompact(metrics.views)} note={metrics.viewsMissingContentCount?`${metrics.viewsMissingContentCount} 則來源未提供觀看，不視為 0`:`${metrics.viewsKnownContentCount} 則內容皆有觀看欄位`}/><Kpi label="有效互動" value={formatCompact(metrics.engagement)} note={`互動率 ${metrics.engagementRate.toFixed(1)}%`}/><Kpi label="民眾對話" value={formatNumber(metrics.conversationCount || metrics.messages)} note={`訊息 ${formatNumber(metrics.messageCount || metrics.messages)} 則`} accent="text-accent"/></div>
    <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-[1.2fr_.8fr]"><Card><ChartHeader title="平台貢獻" eyebrow="Contribution" rows={Object.entries(metrics.platforms).map(([platform,m]) => ({平台:platform,人數型指標:m.people,內容:m.contentCount}))} filename={`${campaign.id}-platform.csv`}/><div className="space-y-5 px-5 pb-6 pt-5">{Object.entries(metrics.platforms).map(([platform,m],index) => <div key={platform}><div className="mb-2 flex justify-between text-[12px]"><span className="font-semibold">{platform}</span><span className="font-mono-ui text-muted-foreground">{formatCompact(m.people)} · {metrics.peopleCount?Math.round(m.people/metrics.peopleCount*100):0}% · {m.contentCount} 則</span></div><BarMeter value={m.people} max={maxContribution} color={index===0?'bg-primary':index===1?'bg-accent':'bg-[hsl(201_45%_44%)]'}/></div>)}</div></Card><Card><ChartHeader title="問題主題" eyebrow="Questions people asked" rows={themes.map(([topic,count]) => ({主題:topic,提問數:count}))} filename={`${campaign.id}-questions.csv`}/><div className="space-y-3 px-5 pb-5 pt-4">{themes.length ? themes.map(([topic,count],index) => <div key={topic} className="flex items-center gap-3"><span className="font-mono-ui text-[11px] text-muted-foreground">{String(index+1).padStart(2,'0')}</span><span className="flex-1 text-[12px]">{topic}</span><b className="font-mono-ui text-[12px]">{count}</b></div>) : <EmptyState title="還沒有歸類提問" body="匯入或同步留言 / 私訊後，這裡會形成問題主題。"/>}</div></Card></div>
    <ThreadsCampaignInsightPanel contents={metrics.contents}/>
    <Card className="mt-4"><ChartHeader title="FB＋IG 同步發佈" eyebrow="Cross-published content" rows={crossGroups.map((group) => ({內容:group.title,平台:group.platforms.join(' + '),發布:group.publishedAt}))} filename={`${campaign.id}-cross-published.csv`}/><div className="divide-y divide-border px-5 pb-2 pt-2">{crossGroups.length ? crossGroups.map((group) => <div key={group.id} className="py-4"><div className="flex flex-wrap items-center gap-2"><Pill tone="good">{group.platforms.join(' + ')}</Pill><p className="min-w-0 flex-1 truncate text-[13px] font-semibold">{group.title}</p><span className="text-[10px] text-muted-foreground">{formatDate(group.publishedAt)}</span></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{group.items.map((item) => <div key={item.id} className="rounded-xl bg-secondary/45 p-3 text-[11px]"><div className="flex items-center justify-between"><b>{item.platform}</b><span className="text-muted-foreground">{item.type}</span></div><div className="mt-2 grid grid-cols-3 gap-2"><span>觀看<b className="mt-1 block text-[12px]">{contentMetricText(item,'views')}</b></span><span>觸及<b className="mt-1 block text-[12px]">{contentMetricText(item,'reach')}</b></span><span>互動<b className="mt-1 block text-[12px]">{formatCompact(item.engagement)}</b></span></div>{item.metricAvailability?.views===false&&<p className="mt-2 text-[10px] text-muted-foreground">這份 {item.platform} 匯出檔未提供逐篇觀看，不代表 0 次觀看。</p>}</div>)}</div></div>) : <EmptyState title="目前沒有辨識到同步發佈組合" body="同一文案在不同平台、15 分鐘內發布時，會在這裡配成一組；平台成效仍各自獨立計算。"/>}</div></Card>
    <Card className="mt-4"><ChartHeader title="活動宣傳內容" eyebrow="逐篇內容" rows={metrics.contents.map((x) => ({標題:x.title,平台:x.platform,人數型指標:peopleMetricValue(x),互動:x.engagement}))} filename={`${campaign.id}-contents.csv`}/><div className="divide-y divide-border px-5 pb-2 pt-2">{metrics.contents.length ? [...metrics.contents].sort((a,b)=>peopleMetricValue(b)-peopleMetricValue(a)).slice(0,10).map((content,index) => <div key={content.id} className="flex items-center gap-3 py-4"><span className="w-5 font-mono-ui text-[11px] text-muted-foreground">{String(index+1).padStart(2,'0')}</span><div className="min-w-0 flex-1"><p className="truncate text-[13px] font-semibold">{content.title}</p><p className="mt-1 text-[11px] text-muted-foreground">{content.platform} · {formatDate(content.publishedAt)} · {formatCompact(peopleMetricValue(content))} {content.platform==='Threads'?'瀏覽人數':'觸及'}</p></div><span className="font-mono-ui text-[12px]">{formatCompact(content.engagement)} 互動</span></div>) : <EmptyState title="活動尚未有內容" body="把內容接受、改派到這個活動後，就會出現在這裡。"/>}</div></Card>
...
function ContentLibrary() {
  ...
  const crossGroups=useMemo(()=>crossPublishedGroups(data.contents),[data.contents]);
  ...
  return <Shell title="內容資料庫"><PageIntro kicker="Content library" title="逐篇內容，這次真的會進來。" description="API 與 CSV / XLSX / JSON 最後都進同一個 merge pipeline；同一篇只更新數字，不會每天長出一份分身。"><div className="flex gap-2"><CsvButton rows={rows.map(x=>({標題:x.title,平台:x.platform,類型:x.type,發布:x.publishedAt,觀看:x.views,觸及:x.reach,互動:x.engagement,狀態:x.reviewStatus}))} filename="content-library.csv"/><button onClick={()=>downloadJson('content-library.json',rows)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-border bg-card px-3 text-[12px] font-semibold"><FileJson className="h-4 w-4"/>JSON</button></div></PageIntro>{crossGroups.length>0&&<Card className="mb-4"><ChartHeader title="FB＋IG 同步發布" eyebrow="Cross-published" rows={crossGroups.map(g=>({內容:g.title,平台:g.platforms.join(' + '),發布:g.publishedAt}))} filename="cross-published-content.csv"/><div className="grid gap-3 px-5 pb-5 pt-2 md:grid-cols-2">{crossGroups.slice(0,8).map(group=><div key={group.id} className="rounded-xl border border-border bg-secondary/20 p-3"><div className="flex items-center gap-2"><Pill tone="good">{group.platforms.join(' + ')}</Pill><span className="text-[10px] text-muted-foreground">{formatDate(group.publishedAt)}</span></div><p className="mt-2 line-clamp-2 text-[12px] font-semibold">{group.title}</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{group.items.map(item=><button type="button" key={item.id} onClick={()=>setSelected(item)} className="rounded-lg bg-card p-2 text-left text-[10px]"><b>{item.platform}</b><div className="mt-1 grid grid-cols-3 gap-1 text-muted-foreground"><span>觀看<strong className="block text-foreground">{contentMetricText(item,'views')}</strong></span><span>觸及<strong className="block text-foreground">{contentMetricText(item,'reach')}</strong></span><span>互動<strong className="block text-foreground">{formatCompact(item.engagement)}</strong></span></div></button>)}</div></div>)}</div><p className="px-5 pb-4 text-[10px] leading-5 text-muted-foreground">這裡只是把同一波 Facebook＋Instagram 內容配成一組方便比較；資料庫仍保留兩篇獨立內容，FB 與 IG 的成效各算各的，不會互相吃掉。</p></Card>}<Card><div className="flex flex-col gap-3 border-b border-border p-4 xl:flex-row xl:items-center"><div className="relative w-full xl:max-w-[330px]"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground"/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="搜尋文案、活動、ID、網址" className="h-10 w-full rounded-lg border border-border bg-background pl-9 pr-3 text-[12px]"/></div><select value={platform} onChange={e=>setPlatform(e.target.value)} className="h-10 rounded-lg border border-border bg-background px-3 text-[12px]"><option>全部平台</option><option>Facebook</option><option>Instagram</option><option>Threads</option></select><select value={type} onChange={e=>setType(e.target.value)} className="h-10 rounded-lg border border-border bg-background px-3 text-[12px]"><option>全部類型</option><option>Post</option><option>Reel</option><option>Story</option><option>Threads Post</option></select><select value={status} onChange={e=>setStatus(e.target.value)} className="h-10 rounded-lg border border-border bg-background px-3 text-[12px]"><option>全部狀態</option><option value="suggested">待覆核</option><option value="accepted">已接受</option><option value="reassigned">已改派</option><option value="excluded">已排除</option></select><select value={campaign} onChange={e=>setCampaign(e.target.value)} className="h-10 rounded-lg border border-border bg-background px-3 text-[12px]"><option>全部活動</option>{data.campaigns.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><span className="ml-auto text-[11px] text-muted-foreground">{rows.length} / {data.contents.length} 則</span></div><div className="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-3">{rows.map(content=><button type="button" key={content.id} onClick={()=>setSelected(content)} className="rounded-xl border border-border bg-background p-4 text-left transition hover:border-primary/40 hover:bg-secondary/20"><div className="flex items-center justify-between gap-2"><Pill tone={content.reviewStatus==='suggested'?'warn':content.reviewStatus==='excluded'?'coral':'good'}>{content.reviewStatus==='suggested'?'待覆核':content.reviewStatus==='accepted'?'已接受':content.reviewStatus==='reassigned'?'已改派':'已排除'}</Pill><span className="text-[10px] text-muted-foreground">{content.platform} · {content.type}</span></div><p className="mt-3 line-clamp-2 text-[13px] font-semibold leading-5">{content.title}</p><p className="mt-2 text-[10px] text-muted-foreground">{content.campaignName} · {formatDate(content.publishedAt)}</p><div className="mt-4 grid grid-cols-3 gap-2 text-[10px]"><div><span className="text-muted-foreground">觀看</span><b className="mt-1 block text-[12px]">{contentMetricText(content,'views')}</b></div><div><span className="text-muted-foreground">{content.platform==='Threads'?'瀏覽人數':'觸及'}</span><b className="mt-1 block text-[12px]">{content.platform==='Threads'?(content.threadsInsights?.viewers===undefined?'未提供':formatCompact(content.threadsInsights.viewers)):contentMetricText(content,'reach')}</b></div><div><span className="text-muted-foreground">互動</span><b className="mt-1 block text-[12px]">{formatCompact(content.engagement)}</b></div></div></button>)}</div></Card></Shell>;
}

function ThreadsCampaignInsightPanel({ contents }: { contents: SocialContent[] }) {
  const rows=contents.filter((item)=>item.platform==='Threads'&&item.threadsInsights);
  if(!rows.length)return null;
  const totalViews=rows.reduce((sum,item)=>sum+Number(item.views||0),0);
  const viewers=rows.reduce((sum,item)=>sum+Number(item.threadsInsights?.viewers||0),0);
  const profileViews=rows.reduce((sum,item)=>sum+Number(item.threadsInsights?.profileViews||0),0);
  const follows=rows.reduce((sum,item)=>sum+Number(item.threadsInsights?.follows||0),0);
  const weightedRate=(key:'like'|'reply'|'share'|'quote'|'repost')=>{let numerator=0,denominator=0;for(const item of rows){const value=item.threadsInsights?.rates?.[key];if(value===undefined)continue;const weight=Number(item.threadsInsights?.viewers||item.views||1);numerator+=value*weight;denominator+=weight;}return denominator?numerator/denominator:undefined;};
  const weightedSource=(key:'home'|'search'|'profile'|'activityTab')=>{let numerator=0,denominator=0;for(const item of rows){const value=item.threadsInsights?.trafficSources?.[key];if(value===undefined)continue;const weight=Number(item.views||1);numerator+=value*weight;denominator+=weight;}return denominator?numerator/denominator:undefined;};
  const rates=[['按讚率',weightedRate('like')],['回覆率',weightedRate('reply')],['分享率',weightedRate('share')],['引用率',weightedRate('quote')],['轉發率',weightedRate('repost')]] as const;
  const sources=[['首頁',weightedSource('home')],['搜尋',weightedSource('search')],['個人檔案',weightedSource('profile')],['活動頁籤',weightedSource('activityTab')]] as const;
  return <Card className="mt-4"><ChartHeader title="Threads 單篇洞察彙整" eyebrow="Per-post Threads insights" rows={rows.map((x)=>({內容:x.title,瀏覽:x.views,瀏覽人數:x.threadsInsights?.viewers??'',追蹤:x.threadsInsights?.follows??'',按讚率:x.threadsInsights?.rates?.like??''}))} filename="threads-campaign-insights.csv"/><div className="grid gap-3 px-5 pb-4 pt-3 sm:grid-cols-4"><div className="rounded-xl bg-primary/5 p-3"><p className="text-[10px] text-muted-foreground">Threads 貼文</p><b className="mt-1 block text-[20px]">{rows.length}</b></div><div className="rounded-xl bg-secondary/45 p-3"><p className="text-[10px] text-muted-foreground">累計瀏覽</p><b className="mt-1 block text-[20px]">{formatCompact(totalViews)}</b></div><div className="rounded-xl bg-secondary/45 p-3"><p className="text-[10px] text-muted-foreground">瀏覽人數（逐篇加總）</p><b className="mt-1 block text-[20px]">{formatCompact(viewers)}</b></div><div className="rounded-xl bg-secondary/45 p-3"><p className="text-[10px] text-muted-foreground">追蹤 / 個人檔案瀏覽</p><b className="mt-1 block text-[20px]">{formatNumber(follows)} / {formatNumber(profileViews)}</b></div></div><div className="grid gap-4 px-5 pb-5 lg:grid-cols-2"><div className="rounded-xl border border-border p-4"><b className="text-[12px]">互動率（依各篇瀏覽人數加權）</b><div className="mt-3 grid grid-cols-5 gap-2">{rates.map(([label,value])=><div key={label} className="text-center"><p className="text-[9px] text-muted-foreground">{label}</p><b className="mt-1 block text-[12px]">{value===undefined?'—':`${value.toFixed(2)}%`}</b></div>)}</div></div><div className="rounded-xl border border-border p-4"><b className="text-[12px]">瀏覽來源（依各篇瀏覽量加權）</b><div className="mt-3 grid grid-cols-4 gap-2">{sources.map(([label,value])=><div key={label} className="text-center"><p className="text-[9px] text-muted-foreground">{label}</p><b className="mt-1 block text-[12px]">{value===undefined?'—':`${value.toFixed(1)}%`}</b></div>)}</div></div></div><p className="px-5 pb-5 text-[10px] leading-5 text-muted-foreground">Threads「瀏覽人數」是每篇貼文的人數欄位加總，不代表整個活動去重後的唯一人數；互動率保留為率，不用百分比反推實際按讚／回覆人數。</p></Card>;
}

function ThreadsInsightPanel({ content }: { content: SocialContent }) {
  const insight = content.threadsInsights;
  if (!insight) return null;
  const rateRows = [
    ['按讚率', insight.rates?.like, insight.benchmarks?.likeRate],
    ['回覆率', insight.rates?.reply, insight.benchmarks?.replyRate],
    ['分享率', insight.rates?.share, insight.benchmarks?.shareRate],
    ['引用率', insight.rates?.quote, insight.benchmarks?.quoteRate],
    ['轉發率', insight.rates?.repost, insight.benchmarks?.repostRate],
  ] as const;
  const sourceRows = [
    ['首頁', insight.trafficSources?.home],
    ['搜尋', insight.trafficSources?.search],
    ['個人檔案', insight.trafficSources?.profile],
    ['活動頁籤', insight.trafficSources?.activityTab],
  ] as const;
  return <Card className="mt-4"><div className="p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11px] font-semibold uppercase tracking-[.16em] text-primary">Threads post insights</p><h2 className="mt-1 text-[16px] font-semibold">單篇 Threads 洞察</h2><p className="mt-1 text-[10px] leading-5 text-muted-foreground">這些欄位來自 Threads 單篇洞察頁；互動率保留百分比，不會反推成人數。</p></div>{insight.accountHandle&&<Pill tone="neutral">@{insight.accountHandle}</Pill>}</div><div className="mt-4 grid gap-3 sm:grid-cols-4"><div className="rounded-xl bg-secondary/45 p-3"><p className="text-[10px] text-muted-foreground">瀏覽次數</p><b className="mt-1 block text-[18px]">{formatNumber(content.views)}</b><small className="text-[10px] text-muted-foreground">{insight.benchmarks?.views??'—'}</small></div><div className="rounded-xl bg-secondary/45 p-3"><p className="text-[10px] text-muted-foreground">瀏覽人數</p><b className="mt-1 block text-[18px]">{insight.viewers===undefined?'未提供':formatNumber(insight.viewers)}</b><small className="text-[10px] text-muted-foreground">{insight.benchmarks?.viewers??'—'}</small></div><div className="rounded-xl bg-secondary/45 p-3"><p className="text-[10px] text-muted-foreground">個人檔案瀏覽</p><b className="mt-1 block text-[18px]">{insight.profileViews===undefined?'未提供':formatNumber(insight.profileViews)}</b><small className="text-[10px] text-muted-foreground">{insight.benchmarks?.profileViews??'—'}</small></div><div className="rounded-xl bg-secondary/45 p-3"><p className="text-[10px] text-muted-foreground">追蹤次數</p><b className="mt-1 block text-[18px]">{insight.follows===undefined?'未提供':formatNumber(insight.follows)}</b><small className="text-[10px] text-muted-foreground">{insight.benchmarks?.follows??'—'}</small></div></div><div className="mt-4 grid gap-4 lg:grid-cols-2"><div className="rounded-xl border border-border p-4"><b className="text-[12px]">影響瀏覽次數的因素</b><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5 lg:grid-cols-3">{rateRows.map(([label,value,benchmark])=><div key={label} className="rounded-lg bg-secondary/35 p-3"><p className="text-[10px] text-muted-foreground">{label}</p><b className="mt-1 block text-[14px]">{value===undefined?'—':`${value.toFixed(2)}%`}</b><span className="text-[9px] text-muted-foreground">{benchmark??''}</span></div>)}</div></div><div className="rounded-xl border border-border p-4"><b className="text-[12px]">瀏覽次數主要來源</b><div className="mt-3 space-y-3">{sourceRows.map(([label,value])=><div key={label}><div className="mb-1 flex justify-between text-[11px]"><span>{label}</span><b>{value===undefined?'—':`${value.toFixed(2)}%`}</b></div>{value!==undefined&&<BarMeter value={value} max={100}/>}</div>)}</div></div></div>{insight.tagNames?.length?<p className="mt-4 text-[10px] text-muted-foreground">活動 / 標籤：{insight.tagNames.join('、')}</p>:null}</div></Card>;
}
...
function ContentDetail({content,onClose}:{content:SocialContent;onClose:()=>void}) {
  ...
  const isThreads=content.platform==='Threads'; const threads=content.threadsInsights;
  const secondLabel=isThreads?'瀏覽人數':'觸及'; const secondValue=isThreads?(threads?.viewers===undefined?'未提供':formatCompact(threads.viewers)):contentMetricText(content,'reach'); const secondNote=isThreads?'Threads 單篇洞察的人數欄位；不是 Meta Reach':content.metricAvailability?.reach===false?'匯出檔未提供逐篇觸及':'目前最新值';
  const thirdLabel=isThreads&&content.metricAvailability?.engagement===false?'按讚率':'互動'; const thirdValue=isThreads&&content.metricAvailability?.engagement===false?(threads?.rates?.like===undefined?'未提供':`${threads.rates.like.toFixed(2)}%`):formatCompact(content.engagement); const thirdNote=isThreads&&content.metricAvailability?.engagement===false?`回覆率 ${threads?.rates?.reply===undefined?'—':`${threads.rates.reply.toFixed(2)}%`} · 不反推互動人數`:`7 日觀看成長 ${g168.percent.toFixed(1)}%`;
  const fourthLabel=isThreads?'追蹤 / 個人檔案瀏覽':'分享 / 收藏'; const fourthValue=isThreads?`${threads?.follows===undefined?'—':formatNumber(threads.follows)} / ${threads?.profileViews===undefined?'—':formatNumber(threads.profileViews)}`:`${formatNumber(content.shares)} / ${formatNumber(content.saves)}`; const fourthNote=isThreads?'來自 Threads 單篇摘要':`點擊 ${formatNumber(content.clicks)}`;
  return <Shell title="內容詳情" section="內容資料庫"><button onClick={onClose} className="mb-4 text-[12px] font-semibold text-primary">← 返回內容資料庫</button><PageIntro kicker={`${content.platform} · ${content.type}`} title={content.title} description={`${content.campaignName} · ${content.publishedAt}`}><ExternalButton href={content.permalink||content.url||'#'}/></PageIntro><div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><Kpi label="目前觀看" value={contentMetricText(content,'views')} note={content.metricAvailability?.views===false?'來源未提供逐篇觀看':`24h ${g24.absolute>=0?'+':''}${formatCompact(g24.absolute)} / ${g24.percent.toFixed(1)}%`}/><Kpi label={secondLabel} value={secondValue} note={secondNote}/><Kpi label={thirdLabel} value={thirdValue} note={thirdNote}/><Kpi label={fourthLabel} value={fourthValue} note={fourthNote}/></div><ThreadsInsightPanel content={content}/><div className="mt-4 grid gap-4 lg:grid-cols-[1.3fr_.7fr]"><Card><ChartHeader title="歷史成長快照" eyebrow="Snapshot history" rows={chart} filename={`${content.id}-snapshots.csv`}/><div className="h-[300px] p-4">{chart.length>1?<ResponsiveContainer width="100%" height="100%"><AreaChart data={chart}><CartesianGrid stroke="hsl(var(--border))" strokeDasharray="3 5" vertical={false}/><XAxis dataKey="time" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} tickFormatter={formatCompact}/><Tooltip content={<TooltipBox/>}/><Area type="monotone" dataKey="views" name="觀看" stroke={COLORS.blue} fill={COLORS.blue} fillOpacity={.12}/>{!isThreads&&<Area type="monotone" dataKey="reach" name="觸及" stroke={COLORS.teal} fill={COLORS.teal} fillOpacity={.08}/>}</AreaChart></ResponsiveContainer>:<EmptyState title="等待下一次快照" body="下一次同步或重新匯入同一篇內容，而且數字有變化時，就會形成歷史曲線。Threads 可用重複擷取同一篇建立觀看成長紀錄。"/>}</div></Card><Card><div className="p-5"><p className="text-[11px] font-semibold uppercase tracking-[.16em] text-primary">Attribution review</p><h2 className="mt-1 text-[16px] font-semibold">活動歸屬覆核</h2><div className="mt-4 rounded-xl bg-secondary/45 p-3 text-[11px] leading-5 text-muted-foreground"><b className="text-foreground">系統建議理由</b><br/>{content.classificationReasons?.length?content.classificationReasons.join('、'):'舊資料尚未重新跑分類器'}<br/>信心：{content.confidence} · 分數 {content.classificationScore??'—'}</div><label className="mt-4 block text-[12px] font-semibold">指定活動<select value={campaignId} onChange={e=>setCampaignId(e.target.value)} className="mt-2 h-10 w-full rounded-lg border border-border bg-background px-3"><option value="unassigned">尚未歸類</option>{data.campaigns.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><div className="mt-4 grid grid-cols-3 gap-2"><button onClick={()=>void saveReview('accepted')} className="h-9 rounded-lg bg-primary text-[11px] font-semibold text-primary-foreground">接受</button><button onClick={()=>void saveReview('reassigned')} className="h-9 rounded-lg bg-accent/15 text-[11px] font-semibold text-accent">改派</button><button onClick={()=>void saveReview('excluded')} className="h-9 rounded-lg bg-destructive/10 text-[11px] font-semibold text-destructive">排除</button></div><p className="mt-3 text-[10px] leading-5 text-muted-foreground">人工覆核會寫入 IndexedDB；之後 API / CSV / Threads 擷取更新 metrics 時不會把這個決定洗掉。</p></div></Card></div></Shell>;
}
...
function Platforms() {
  const {data}=useSocialDataset();
  const rows=['Facebook','Instagram','Threads'].map(platform=>{
    const contents=data.contents.filter(x=>x.platform===platform&&x.reviewStatus!=='excluded');
    const original=data.platforms.find(x=>x.platform===platform);
    const hasViews=contents.some(x=>metricDisplay(x,'views').available)||Number(original?.views||0)>0;
    const hasReach=contents.some(x=>metricDisplay(x,'reach').available)||Number(original?.reach||0)>0;
    const people=contents.reduce((sum,x)=>sum+peopleMetricValue(x),0)||original?.reach||0; return {platform,followers:original?.followers??0,growth:original?.growth??0,views:contents.reduce((s,x)=>s+x.views,0)||original?.views||0,reach:contents.reduce((s,x)=>s+x.reach,0)||original?.reach||0,people,engagement:contents.reduce((s,x)=>s+x.engagement,0)||original?.engagement||0,posts:contents.filter(x=>x.type==='Post'||x.type==='Threads Post').length,reels:contents.filter(x=>x.type==='Reel').length,stories:contents.filter(x=>x.type==='Story').length,messages:contents.reduce((s,x)=>s+x.messages,0)||original?.messages||0,hasViews,hasReach:people>0};
  });
  const crossGroups=crossPublishedGroups(data.contents);
  const crossTotals=crossGroups.reduce((acc,g)=>({views:acc.views+g.views,reach:acc.reach+g.reach,engagement:acc.engagement+g.engagement}),{views:0,reach:0,engagement:0});
  return <Shell title="平台比較"><PageIntro kicker="Platforms" title="三個平台，各自擅長什麼？" description="平台數字優先由逐篇內容加總；FB＋IG 同步發佈會另外配成『跨平台共同發布』，但兩邊成效仍各自保留，不會只算其中一邊。"/>
    <div className="grid gap-4 lg:grid-cols-3">{rows.map(row=><Card key={row.platform}><div className="p-5"><div className="flex items-center justify-between"><h2 className="text-[17px] font-semibold">{row.platform}</h2><span className="h-3 w-3 rounded-full" style={{backgroundColor:platformColors[row.platform]}}/></div><p className="mt-5 font-display text-[36px]">{row.hasReach?formatCompact(row.people):'未提供'}</p><p className="text-[11px] text-muted-foreground">{row.hasReach?(row.platform==='Threads'?'Threads 瀏覽人數（逐篇加總）':'觸及人數（逐篇加總）'):'這份資料來源沒有逐篇人數型指標'}</p><div className="mt-5 grid grid-cols-2 gap-3 text-[11px]"><div className="rounded-xl bg-secondary/45 p-3">觀看<b className="mt-1 block text-[14px]">{row.hasViews?formatCompact(row.views):'未提供'}</b></div><div className="rounded-xl bg-secondary/45 p-3">互動<b className="mt-1 block text-[14px]">{formatCompact(row.engagement)}</b></div><div className="rounded-xl bg-secondary/45 p-3">貼文 / Threads<b className="mt-1 block text-[14px]">{row.posts}</b></div><div className="rounded-xl bg-secondary/45 p-3">Reels / Story<b className="mt-1 block text-[14px]">{row.reels} / {row.stories}</b></div></div></div></Card>)}</div>
    <Card className="mt-4"><ChartHeader title="FB＋IG 跨平台共同發布" eyebrow="Same creative, separate metrics" rows={crossGroups.map(g=>({內容:g.title,平台:g.platforms.join(' + '),觀看:g.views,人數型觸及:g.reach,互動:g.engagement}))} filename="cross-published-content.csv"/><div className="grid gap-3 p-5 sm:grid-cols-3"><div className="rounded-xl bg-primary/5 p-4"><p className="text-[10px] text-muted-foreground">共同發布素材</p><b className="mt-1 block font-display text-[27px]">{crossGroups.length}</b><p className="mt-1 text-[10px] text-muted-foreground">1 組素材可包含 FB＋IG 兩筆平台內容</p></div><div className="rounded-xl bg-secondary/50 p-4"><p className="text-[10px] text-muted-foreground">跨平台累計觀看</p><b className="mt-1 block font-display text-[27px]">{formatCompact(crossTotals.views)}</b><p className="mt-1 text-[10px] text-muted-foreground">FB 與 IG 各自觀看加總</p></div><div className="rounded-xl bg-secondary/50 p-4"><p className="text-[10px] text-muted-foreground">跨平台累計人數型指標</p><b className="mt-1 block font-display text-[27px]">{formatCompact(crossTotals.reach)}</b><p className="mt-1 text-[10px] text-muted-foreground">只是兩平台算術加總，不代表跨平台去重人數</p></div></div><div className="divide-y divide-border px-5 pb-3">{crossGroups.length?crossGroups.slice(0,20).map(group=><div key={group.id} className="py-4"><div className="flex flex-wrap items-center gap-2"><Pill tone="good">FB + IG</Pill><p className="min-w-0 flex-1 truncate text-[12px] font-semibold">{group.title}</p><span className="text-[10px] text-muted-foreground">文案相似度 {Math.round(group.similarity*100)}%</span></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{group.items.map(item=><div key={item.id} className="rounded-xl bg-secondary/40 p-3 text-[11px]"><div className="flex justify-between"><b>{item.platform}</b><span>{item.type}</span></div><div className="mt-2 grid grid-cols-3 gap-2"><span>觀看<b className="mt-1 block">{contentMetricText(item,'views')}</b></span><span>人數<b className="mt-1 block">{contentMetricText(item,'reach')}</b></span><span>互動<b className="mt-1 block">{formatCompact(item.engagement)}</b></span></div></div>)}</div></div>):<EmptyState title="還沒有辨識到共同發布" body="匯入 FB＋IG 真實報告後，系統會依發布時間與文案相似度自動配對。"/>}</div></Card>
    <Card className="mt-4"><ChartHeader title="平台觸及與互動" eyebrow="Comparison" rows={rows.map(x=>({平台:x.platform,人數型指標:x.people,互動:x.engagement}))} filename="platform-comparison.csv"/><div className="h-[330px] p-5"><ResponsiveContainer width="100%" height="100%"><BarChart data={rows}><CartesianGrid stroke="hsl(var(--border))" strokeDasharray="3 5" vertical={false}/><XAxis dataKey="platform"/><YAxis tickFormatter={formatCompact}/><Tooltip content={<TooltipBox/>}/><Legend/><Bar dataKey="people" name="人數型指標" fill={COLORS.teal} radius={[5,5,0,0]}/><Bar dataKey="engagement" name="互動" fill={COLORS.coral} radius={[5,5,0,0]}/></BarChart></ResponsiveContainer></div></Card>
  </Shell>;
}
...
function Reports() {
  ...
  const reach = m?.peopleCount ?? contents.reduce((s, x) => s + peopleMetricValue(x), 0);
...
本期跨 Facebook、Instagram 與 Threads 共彙整 ${contents.length} 則有效內容，累計觀看／曝光 ${formatNumber(views)}、人數型指標 ${formatNumber(reach)}（FB/IG 觸及＋Threads 瀏覽人數，逐篇加總且非跨平台去重）、互動 ${formatNumber(eng)}，並記錄 ${formatNumber(conversationCount)} 組有效對話及 ${formatNumber(messages)} 則內容或私訊相關訊息。

${topContent ? `成效較突出的內容為「${topContent.title}」，人數型指標 ${formatNumber(peopleMetricValue(topContent))}、互動 ${formatNumber(topContent.engagement)}。` : '本期尚無可排名的逐篇內容。'}

${!campaign && topCampaign?.m.contentCount ? `活動層級以「${topCampaign.c.name}」人數型指標表現較高，共納入 ${topCampaign.m.contentCount} 則有效內容、累計 ${formatNumber(topCampaign.m.peopleCount)}。` : ''}
...
整體而言，跨平台內容已形成可追蹤、可覆核的宣傳與服務紀錄；後續可持續利用內容快照、活動歸因與詢問分類，作為下一年度宣傳策略、資訊優化及經費需求之佐證。`;
  ...
  return <Shell title="報告中心"><PageIntro kicker="Reports" title="把數據，翻成可以送進成果報告的話。" description="年度、季度、月份、自訂日期與單一活動都會真的過濾資料，不只是換下拉選單文字。"><div className="flex gap-2"><PrintButton/><button onClick={copy} className="inline-flex h-9 items-center gap-2 rounded-lg border border-border bg-card px-3 text-[12px] font-semibold"><Copy className="h-4 w-4"/>複製文字</button></div></PageIntro>
    ...
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><Kpi label="已取得觀看 / 曝光" value={formatCompact(views)} note={`${contents.filter(x=>metricDisplay(x,'views').available).length}/${contents.length} 則有觀看欄位 · ${periodLabel}`}/><Kpi label="人數型指標" value={formatCompact(reach)} note="FB/IG 觸及＋Threads 瀏覽人數；非去重"/><Kpi label="互動" value={formatCompact(eng)} note={`${contents.length} 則內容`}/><Kpi label="有效對話" value={formatNumber(conversationCount)} note={`訊息 ${formatNumber(messages)} 則`} accent="text-accent"/></div>
...
function DataCenter() {
  ...
  const isThreadsCapture = (value: unknown): value is ThreadsCapturePayload => {
    if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
    return (value as ThreadsCapturePayload).captureType === 'threads-insights-capture';
  };
  ...
  const previewThreadsCapture = (capture: ThreadsCapturePayload, sourceLabel = 'Threads Chrome 擷取器') => {
    const parsed = parseThreadsCapturePayload(capture);
    setFileName(sourceLabel); setSource('threads-extension-capture'); setKind('contents');
    setIncoming({ contents: parsed.contents, isDemo: false });
    setWarnings([`${sourceLabel}：${capture.capturedAt ? `擷取於 ${capture.capturedAt}` : '已接收目前頁面資料'}`, ...parsed.warnings]);
    setPreviewRows([['平台','內容','發布日','觀看','瀏覽人數','互動'], ...parsed.contents.slice(0, 5).map((row) => [row.platform,row.title,row.publishedAt,String(row.views),row.threadsInsights?.viewers===undefined?'—':String(row.threadsInsights.viewers),row.metricAvailability?.engagement===false?'僅有互動率':String(row.engagement)])]);
    ...
    setResult('Threads 洞察已從瀏覽器擷取器送進 Dashboard；確認預覽後按「確認合併」即可。');
  };

  useEffect(() => {
    const receiveBridge = (event: MessageEvent) => {
      if (event.source !== window || event.data?.type !== 'SOCIAL_IMPACT_THREADS_CAPTURE') return;
      const capture = event.data?.payload as ThreadsCapturePayload | undefined;
      if (!capture || capture.captureType !== 'threads-insights-capture') return;
      previewThreadsCapture(capture, 'Threads 擷取器直接傳入');
    };
  ...
  const parseImport = async (file: File) => {
    ...
    if (meta) {
      ...
      setCampaignHints(discovered.evidence.map((x) => `${x.name}（${x.count} 則內容）`));
      setPreviewRows([['平台','內容','發布日','類型','觀看','觸及','互動'], ...meta.contents.slice(0, 5).map((row) => [row.platform, row.title, row.publishedAt, row.type, row.metricAvailability?.views===false?'未提供':String(row.views), row.metricAvailability?.reach===false?'未提供':String(row.reach), String(row.engagement)])]);
      return;
    }
  ...
  return <Shell title="資料中心"><PageIntro kicker="Data center" title="資料進來之前，先讓它可被信任。" description="手動匯入與 API 同步使用同一套去重、快照與人工覆核保護規則。CSV / XLSX 會先判斷資料類型，再讓你確認合併。"><label className="inline-flex h-10 cursor-pointer items-center gap-2 rounded-lg bg-primary px-4 text-[12px] font-semibold text-primary-foreground"><Upload className="h-4 w-4"/>匯入 CSV / XLSX / JSON<input type="file" accept=".csv,.xlsx,.json,.txt" onChange={handleFile} className="hidden"/></label></PageIntro>
    ...
    <Card className="mt-4"><div className="p-5"><div><p className="text-[11px] font-semibold uppercase tracking-[.16em] text-muted-foreground">Web / conversation fallback</p><h2 className="mt-1 text-[16px] font-semibold">Threads 網頁與 Messenger 手動對話備援</h2><p className="mt-2 text-[11px] leading-5 text-muted-foreground">Threads 沒有匯出按鈕時，優先使用 OAuth/API；尚未完成 OAuth 時就用 Chrome 擷取器直接讀已登入的洞察頁。新版不需要複製文字，也不必先下載 JSON：先在 Threads 擷取，再切回 Dashboard 按擴充套件「送入目前 Dashboard」。Messenger 仍以人工整理對話紀錄作為民眾疑問分析來源。</p></div><div className="mt-5 grid gap-4 lg:grid-cols-2"><div className="rounded-xl border border-border p-4"><b className="text-[12px]">Threads 洞察頁</b><p className="mt-1 text-[10px] leading-5 text-muted-foreground">Threads 洞察頁不能複製也沒關係。Chrome 擷取器 v1.2 會直接讀目前已載入的 DOM。建議先打開某一篇貼文的「洞察」詳情，按①擷取；換下一篇可重複累積。全部抓完後切回 Dashboard，按②一次送入。它會保存瀏覽次數、瀏覽人數、個人檔案瀏覽、追蹤、互動率與瀏覽來源；不讀密碼、Cookie 或 Access Token。</p><div className="mt-3 rounded-xl bg-primary/5 p-3 text-[10px] leading-5 text-muted-foreground"><b className="text-primary">第一次安裝：</b>下載擷取器 → 解壓縮 → Chrome 開啟 chrome://extensions/ → 開啟開發人員模式 → 載入未封裝項目。之後流程：單篇洞察按「①擷取／累積」→ 換下一篇重複 → 全部完成後 Dashboard 按「②全部送入」。</div><div className="mt-3 flex flex-wrap gap-2"><a href={`${import.meta.env.BASE_URL}tools/threads-insights-capture-extension.zip`} download className="inline-flex h-9 items-center rounded-lg bg-primary px-3 text-[11px] font-semibold text-primary-foreground">下載 Threads 擷取器</a><a href="https://www.threads.com/insights?hl=zh-tw" target="_blank" rel="noreferrer" className="inline-flex h-9 items-center rounded-lg border border-border px-3 text-[11px] font-semibold">開啟 Threads Insights</a></div><details className="mt-3"><summary className="cursor-pointer text-[10px] font-semibold text-muted-foreground">如果擷取器暫時不能用，仍可展開文字備援</summary><textarea value={threadsPaste} onChange={e=>setThreadsPaste(e.target.value)} placeholder="貼上 Threads 洞察頁文字…" className="mt-3 min-h-[120px] w-full rounded-xl border border-border bg-background p-3 text-[11px] leading-5"/><button onClick={previewThreadsPaste} className="mt-2 h-9 rounded-lg border border-border px-3 text-[11px] font-semibold">解析文字備援</button></details></div><div className="rounded-xl border border-border p-4"><b className="text-[12px]">Messenger 民眾詢問</b><p className="mt-1 text-[10px] leading-5 text-muted-foreground">把同一位民眾的一段對話貼在同一區塊，不同民眾之間空一行。系統會保存 conversation_count=1，message_count 則依該區塊行數計算。</p><textarea value={messengerPaste} onChange={e=>setMessengerPaste(e.target.value)} placeholder={"民眾A：請問還能報名嗎？\n民眾A：如果額滿可以候補嗎？\n\n民眾B：衛武營附近有停車場嗎？"} className="mt-3 min-h-[150px] w-full rounded-xl border border-border bg-background p-3 text-[11px] leading-5"/><button onClick={previewMessengerPaste} className="mt-3 h-9 rounded-lg bg-accent px-3 text-[11px] font-semibold text-white">解析並預覽對話</button></div></div></div></Card>
...
function Settings() {
  const {status,syncing,syncMessage,syncNow}=useSocialDataset(); const [backend,setBackend]=useState(getBackendUrl()); const [tested,setTested]=useState(''); const [remoteStatus,setRemoteStatus]=useState<DashboardStatus|null>(null); const [oauthInfo,setOauthInfo]=useState<OAuthPublicConfig|null>(null);
  const test=async()=>{setBackendUrl(backend);try{const [s,o]=await Promise.all([fetchBackendStatus(),fetchOAuthPublicConfig()]);setRemoteStatus(s);setOauthInfo(o);setTested('連線成功，OAuth 設定資訊已載入');}catch(error){setTested(`連線失敗：${error instanceof Error?error.message:'未知錯誤'}`)}}; const sync=async()=>{try{await syncNow()}catch(error){setTested(error instanceof Error?error.message:'同步失敗')}}; const shown=remoteStatus??status;
