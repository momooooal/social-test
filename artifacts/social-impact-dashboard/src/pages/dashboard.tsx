...
import { detectImportKind, normalizeImportedContent, normalizeImportedInteraction, normalizeImportedMonthlyMetric, normalizeImportedPlatformMetric, type ImportKind } from '@/lib/importer';
...
function DataCenter() {
  ...
  const resetPreview = () => { setFileName(''); setPreviewRows([]); setIncoming(null); setWarnings([]); setResult(''); setKind('contents'); };
  ...
  const parseImport = async (file: File) => {
    ...
    setIncoming({ contents: normalized.map((x) => x.content).filter((x): x is SocialContent => Boolean(x)), isDemo: false });
